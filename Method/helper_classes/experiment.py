...
class Experiment:
            ...
            for e in range(epochs):
                ...
                for x, y_numerical, target_sequence in self.get_batch(data_train[0], data_train[1], data_train[2], batch_size=cs_batch_size):
                    ...
                    if not synthesizer.name in ['LSTM_As_MT', 'GRU_As_MT']:
                        ...
                        cs_loss = synthesizer(x, target)
                        pred_sequence = ['⊔ ⊓']*x.shape[0]
                        # We are just training, actual accuracy is not computed
                        #pred_sequence = synthesizer(x) # for checking prediction performance
            ...
            for e in range(epochs):
                ...
                for x, y_numerical, target_sequence in self.cs.dataloader.load(Emb, data=data_train, batch_size=cs_batch_size, shuffle=True):
                    ...
                    if not synthesizer.name in ['LSTM_As_MT', 'GRU_As_MT']:
                        ...
                        target = list(map(lambda x: self.cs.dataloader.decompose(x), target_sequence))
                        cs_loss = synthesizer(x, target)
                        pred_sequence = pred_sequence = ['⊔ ⊓']*x.shape[0]
                        #pred_sequence = synthesizer(x)
                    s_acc, h_acc = self.compute_accuracy(pred_sequence, target_sequence)
                    soft_acc.append(s_acc); hard_acc.append(h_acc)
                    if include_embedding_loss:
    ...
    def train_all_nets(self, List_nets, data_train, data_test, epochs=200, cs_batch_size=64, tc_batch_size=512, kf_n_splits=10, cross_validate=False, test=False, save_model = False, include_embedding_loss=False, optimizer = 'Adam', tc_label_smoothing=0.9, record_runtime=False, final=False):
        ...
        if cross_validate:
            ...
            it = 0
            for crv in Training_data['soft acc']:
                mk = Markers[it]
                c = Colors[it]
                plt.plot(crv, mk, markersize=5, color=c)
                it += 1
            plt.legend(tuple(List_nets))
            plt.xlabel("Number of epochs")
            plt.ylabel("Soft Accuracy (%)")
            plt.savefig(self.kwargs['path_to_triples'].split("Triples")[0]+"Training_curves/train_soft_acc.pdf")
            plt.close()
            
            it = 0
            for crv in Training_data['hard acc']:
                mk = Markers[it]
                c = Colors[it]
                plt.plot(crv, mk, markersize=5, color=c)
                it += 1
            ...
            plt.savefig(self.kwargs['path_to_triples'].split("Triples")[0]+"Training_curves/train_hard_acc.pdf")
            plt.close()
            ...
            plt.xlabel("Number of epochs")
            plt.ylabel("Loss")
            plt.savefig(self.kwargs['path_to_triples'].split("Triples")[0]+"Training_curves/train_loss.pdf")
            plt.close()
            
            it = 0
            for crv in Validation_data['soft acc']:
                mk = Markers[it]
                c = Colors[it]
                plt.plot(crv, mk, markersize=5, color=c)
                it += 1
            ...
            plt.xlabel("Number of epochs")
            plt.ylabel("Soft Accuracy (%)")
            plt.savefig(self.kwargs['path_to_triples'].split("Triples")[0]+"Training_curves/val_soft_acc.pdf")
            plt.close()
            
            it = 0
            for crv in Validation_data['hard acc']:
                mk = Markers[it]
                c = Colors[it]
                plt.plot(crv, mk, markersize=5, color=c)
                it += 1
            ...
            plt.xlabel("Number of epochs")
            plt.ylabel("Hard Accuracy (%)")
            plt.savefig(self.kwargs['path_to_triples'].split("Triples")[0]+"Training_curves/val_hard_acc.pdf")
            plt.close()
            ...
            plt.xlabel("Number of epochs")
            plt.ylabel("Loss")
            plt.savefig(self.kwargs['path_to_triples'].split("Triples")[0]+"Training_curves/val_loss.pdf")
            plt.close()
            ...
            plt.xlabel("Number of epochs")
            plt.ylabel("Soft Accuracy (%)")
            plt.savefig(self.kwargs['path_to_triples'].split("Triples")[0]+"Training_curves/no_val_train_soft_acc.pdf")
            plt.close()
            ...
            plt.xlabel("Number of epochs")
            plt.ylabel("Hard Accuracy (%)")
            plt.savefig(self.kwargs['path_to_triples'].split("Triples")[0]+"Training_curves/no_val_train_hard_acc.pdf")
            plt.close()
            ...
            plt.xlabel("Number of epochs")
            plt.ylabel("Loss")
            plt.savefig(self.kwargs['path_to_triples'].split("Triples")[0]+"Training_curves/no_val_train_loss.pdf")
            plt.close()
