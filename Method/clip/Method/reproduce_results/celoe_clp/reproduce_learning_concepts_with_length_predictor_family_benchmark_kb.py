...
data_path = base_path+"Datasets/family-benchmark/Learning_problems/Data.json"
with open(data_path, "r") as file:
    data = json.load(file)
data = list(data.items())

path_to_triples = base_path+"Datasets/family-benchmark/Triples/"
triples = Data({"path_to_triples":path_to_triples})

as_classification = True
kb_path = base_path+"Datasets/family-benchmark/family-benchmark_rich_background.owl"
kb = KnowledgeBase(path=kb_path)
rho = ExpressRefinement(kb)
prefix = list(kb.individuals())[0].get_iri().as_str()
prefix = prefix[:prefix.rfind("/")+1]
all_inds = set(kb.individuals())
...
if __name__=='__main__':
    ...
    parser.add_argument('--max_num_lp', type=int, default=200, help="The maximum number of learning problems to solve")
    ...
    kwargs = {'knowledge_base': kb,
                     ...
                     "learner_name":"GRU", "emb_model_name":"", "pretrained_embedding_path":base_path+"Datasets/family-benchmark/Model_weights/ConEx_GRU.pt",
                     "pretrained_length_learner":base_path+"Datasets/family-benchmark/Model_weights/GRU.pt",
                     ...
                     'linear_hidden':200, 'out_size':0, 'dropout_prob': 0.1, 'num_units':500,
    ...
    results = {}
    random.seed(142)
    _, learning_problems = train_test_split(data, test_size=0.2, random_state=123)
    learning_problems = random.sample(learning_problems, args.max_num_lp)
    print("#"*50)
    ...
    with open(base_path+"Datasets/family-benchmark/Results/concept_learning_results_celoe_clp.json", "w") as results_file:
        json.dump(results, results_file, ensure_ascii=False, indent=3)
    with open(base_path+"Datasets/family-benchmark/Results/concept_learning_avg_results_celoe_clp.json", "w") as avg_results_file:
